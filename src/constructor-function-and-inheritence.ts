...
Computer.prototype.switch = function () {
	console.log(`${this.name} is ${this.isOn ? "off" : "on"}`);
	this.isOn = !this.isOn;
};

function Laptop(name, cpu, memory, brand) {
	Computer.call(this, name, cpu);
	this.memory = memory;
	this.brand = brand;
}
...
Laptop.prototype.open = function () {
	console.log(`Laptop ${this.brand} ${this.name} is opening...`);
};
Laptop.prototype.close = function () {
	console.log(`Laptop ${this.brand} ${this.name} is closing...`);
};
...
PC.prototype = Object.create(Computer.prototype); // {}
PC.prototype.constructor = PC;
PC.prototype.freeze = function () {
	console.log(`PC ${this.brand} ${this.name} is freezing...`);
};

const laptop = new Laptop("Macbook Pro", "M1", "1TB", "Apple");
const pc = new PC("Gaming PC", "Intel Core-i9", "Dell");
